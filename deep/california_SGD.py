...
model = SGDRegressor(
    max_iter=1000, # 최대 반복 횟수 (너무 크면 과적합, 메모리 낭비/ 너무 작으면 수렴 안함)
    tol=0.001, # 손실이 tol보다 작아지면 학습 중단 (수렴 기준) - 더 작게
    eta0=0.01, # 학습률을 더키우고
    learning_rate='invscaling', # 학습률 조정 방법 (상수로 설정)-> 점차 감소(안정적적)
    penalty='l2', # 정규화 없음 (과적합 방지 설정 사용 안함) -> 정규화 추가
    alpha=0.0001, # 정규화 강도
    random_state=42 # 결과 재현을 위한 시드 고정
)

# (추가) 이상치 제거
X_train_scaled = np.delete(X_train_scaled, 4, axis=0)
y_train = np.delete(y_train, 8, axis=0)
